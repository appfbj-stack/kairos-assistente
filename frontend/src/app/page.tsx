...
export default function Home() {
  ...
  const [trialApp, setTrialApp] = useState("");
  ...
  async function createAdminTrial() {
    if (!trialName || !trialApp) return;
    await adminApi().post("/license/create-trial", { client_name: trialName, app_slug: trialApp, company: "", email: "", phone: "" });
    setTrialName(""); setTrialApp("");
    loadAdmin();
  }
          ...
          <div className="h-full flex flex-col">
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
              ...
              <div className="bg-white p-3 rounded-xl shadow-sm border space-y-2">
                <h3 className="text-sm font-semibold">⚡ Trial Rápido</h3>
                <div className="flex gap-2">
                  <input value={trialName} onChange={(e) => setTrialName(e.target.value)} placeholder="Nome do cliente" className="flex-1 px-3 py-2 rounded-lg border text-sm" />
                  <select value={trialApp} onChange={(e) => setTrialApp(e.target.value)} className="px-3 py-2 rounded-lg border text-sm">
                    <option value="">App</option>
                    {adminApps.map((a: any) => <option key={a.id} value={a.slug}>{a.name}</option>)}
                  </select>
                  <button onClick={createAdminTrial} className="px-4 py-2 bg-yellow-500 text-white rounded-lg text-sm font-medium">+ Trial</button>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                {adminClients.map((c: any) => {
                  const clientLicenses = adminLicenses.filter((l: any) => l.client_id === c.id);
                  return (
                    <div key={c.id} className="flex items-center gap-2 bg-white p-2.5 rounded-lg border text-sm hover:shadow-sm transition-shadow">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-1 mt-1 flex-wrap">
                          {adminApps.map((a: any) => {
                            const hasLicense = clientLicenses.some((l: any) => l.app_id === a.id);
                            return (
                              <span key={a.id} className={`text-[10px] px-1.5 py-0.5 rounded ${hasLicense ? "bg-green-50 text-green-600 border border-green-200" : "bg-gray-50 text-gray-300 border border-gray-100"}`}>
                                {hasLicense ? "✅" : "○"} {a.name}
                              </span>
                            );
                          })}
