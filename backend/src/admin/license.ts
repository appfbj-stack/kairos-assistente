...
async function addLog(client_id: string | null, app_id: string | null, action: string, details = "") {
  await runSql(
    "INSERT INTO logs (id, client_id, app_id, action, details) VALUES (?, ?, ?, ?, ?)",
...
router.post("/create-trial", async (req: Request, res: Response) => {
  ...
  const app = await queryOne("SELECT * FROM apps WHERE slug = ?", [app_slug]);
  if (!app) return res.status(400).json({ error: "App não encontrado" });

  let client = email ? await queryOne("SELECT * FROM clients WHERE email = ?", [email]) : null;
  if (!client) {
  ...
  const existingLicense = await queryOne(
    "SELECT * FROM licenses WHERE client_id = ? AND app_id = ?",
    [client.id, app.id]
  );
  if (existingLicense) {
  ...
  await runSql(
    "INSERT INTO licenses (id, client_id, app_id, status, type, start_date, end_date) VALUES (?, ?, ?, 'trial', 'temporary', ?, ?)",
    [licenseId, client.id, app.id, startDate, endDate]
  );

  await addLog(client.id, app.id, "trial_created", `Licença trial de 10 dias para ${client_name}`);
...
router.get("/verify", async (req: Request, res: Response) => {
  ...
  const license = await queryOne(
    "SELECT l.*, c.name as client_name, c.status as client_status FROM licenses l JOIN clients c ON l.client_id = c.id WHERE l.client_id = ? AND l.app_id = ?",
    [client_id, app.id]
  );
...
router.post("/activate", async (req: Request, res: Response) => {
  ...
  const license = await queryOne("SELECT * FROM licenses WHERE id = ?", [license_id]);
  if (!license) return res.status(404).json({ error: "Licença não encontrada" });

  await runSql("UPDATE licenses SET status = 'active', type = 'permanent', updated_at = NOW() WHERE id = ?", [license_id]);
  await runSql("UPDATE clients SET status = 'active', updated_at = NOW() WHERE id = ?", [license.client_id]);

  if (amount) {
    await runSql(
      "INSERT INTO payments (id, client_id, license_id, amount, method, status) VALUES (?, ?, ?, ?, ?, 'confirmed')",
      [crypto.randomUUID(), license.client_id, license_id, amount, method || ""]
    );
  }

  await addLog(license.client_id, license.app_id, "license_activated", `Licença ativada${amount ? ` - R$${amount}` : ""}`);
...
router.post("/check-expired", async (_req: Request, res: Response) => {
  const now = new Date().toISOString().slice(0, 19).replace("T", " ");
  const expired = await queryAll("SELECT * FROM licenses WHERE status = 'trial' AND end_date < ?", [now]);
  for (const l of expired) {
    await runSql("UPDATE licenses SET status = 'expired', updated_at = NOW() WHERE id = ?", [(l as any).id]);
    await addLog((l as any).client_id, (l as any).app_id, "license_expired", "Licença expirada automaticamente");
  }
