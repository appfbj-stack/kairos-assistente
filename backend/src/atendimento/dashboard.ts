...
router.get("/:empresaId", requireCoreAuth, async (req: Request, res: Response) => {
  ...
  const conversationTrend = await queryAll(
    "SELECT DATE(started_at) AS date, COUNT(*) AS total FROM atendimento_conversations WHERE empresa_id = ? AND started_at >= DATE('now', '-7 days') GROUP BY DATE(started_at) ORDER BY date",
    [empresaId]
  );
