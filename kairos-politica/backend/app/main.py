...
def _seed_admin():
    ...
    try:
        ...
        if not tenant:
            tenant = Tenant(name="Gabinete", slug="gabinete", active=True)
...
from app.routes import agenda, auth, cidadaos, dashboard, demandas, liderancas, parceiros, users

app = FastAPI(title="Kairos Política API", version="1.0.0", lifespan=lifespan)
...
app.include_router(cidadaos.router)
app.include_router(liderancas.router)
...
@app.get("/health")
def health():
    return {"status": "ok", "app": "Kairos Política API"}
